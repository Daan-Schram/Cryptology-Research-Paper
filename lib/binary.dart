...
class _BinaryState extends State<Binary> {
  ...
  void binaryBrain() {
    if (switchVal == false){
      if (plaintext != ""){
        var binstring = plaintext.codeUnits.map((x) => x.toRadixString(2).padLeft(8, '0'))
            .join(" ");
        setState(() {
          showText = binstring;
          bottomVisible = true;
      ...
      if (plaintext != ""){
        setState(() {
          var listLetters = List<String>();
          List<String> binlist = plaintext.split(" ");
          binlist.forEach((element) {
            int number = int.parse(element, radix: 2);
            String letter = String.fromCharCode(number);
            listLetters.add(letter);
          });
          showText = listLetters.join("");
          bottomVisible = true;
        });
      }
      else{
