...
class HmacSha2 extends StatefulWidget {

  const HmacSha2({key}) : super(key: key);

  @override
  _HmacSha2State createState() => _HmacSha2State();
}

class _HmacSha2State extends State<HmacSha2> {
  ...
  Future<void> hmacBrain() async {
    if (key == "" || plaintext == ""){
      ...
      final mac224 = await Hmac(sha224).calculateMac(
        message,
        secretKey:secretKey,
      );
      final mac256 = await Hmac(sha256).calculateMac(
        message,
        secretKey:secretKey,
      );
      final mac384 = await Hmac(sha384).calculateMac(
        message,
        secretKey:secretKey,
      );
      final mac512 = await Hmac(sha512).calculateMac(
        message,
        secretKey:secretKey,
      );
      setState(() {
        errorVisible = false;
        showText224 = base64.encode(mac224.bytes);
        showText256 = base64.encode(mac256.bytes);
        showText384 = base64.encode(mac384.bytes);
        showText512 = base64.encode(mac512.bytes);
        bottomVisible = true;
      });
    }
  }
  ...
  _showSnackBarCopyButton() {
    String message = "";
    if (showText224 == "" || showText256 == "" || showText384 == "" || showText512 == ""){
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "This function takes an input and uses the SHA-2 hash functions to generate a MAC.",
                      style: kNormalTextStyle,
                    ),
                  ],
                ),
